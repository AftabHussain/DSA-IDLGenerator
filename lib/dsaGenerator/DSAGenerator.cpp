...
void printOffsets(DSNode *node, std::string indentation, std::ofstream *file,
  ...
  *file << indentation << "Write\n";
  for (DSNode::const_offset_iterator it = node->write_offset_begin();
       it != node->write_offset_end(); it++) {
    *file << indentation << *it << " ";
  }
  *file << "\n";
  *file << indentation << "Read\n";
  for (DSNode::const_offset_iterator it = node->read_offset_begin();
       it != node->read_offset_end(); it++) {
    *file << indentation << *it << " ";
  }
  *file << "\n";
...
bool DSAGenerator::runOnModule(Module &m) {
  ...
  for (auto &F : m) {
    if (F.getName().find("llvm") == std::string::npos) {
      ...
      for (Function::arg_iterator A = F.arg_begin(); A != F.arg_end();
           A++, paramNumber++) {
        ...
        if (!val || !val->getType()->isPointerTy()) {
          ...
          for (DSNode::const_edge_iterator ei = node->edge_begin();
            ...
            if (std::find(visitedNodes.begin(), visitedNodes.end(),
              ...
              continue;
            file << indentation << "{\noffset: " << (*ei).first << "\n";
            printOffsets((*ei).second.getNode(), indentation, &file,
                         &visitedNodes);
            file << "}\n\n";
          }
        }
